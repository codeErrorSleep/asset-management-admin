...
type (
	...
	Permission struct {
		...
		KeepAlive   sql.NullInt64  `db:"keepAlive"`
		Method      sql.NullString `db:"method"`
		Description sql.NullString `db:"description"`
		Show        int64          `db:"show"` // 是否展示在页面菜单
...
func (m *defaultPermissionModel) Insert(ctx context.Context, data *Permission) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Name, data.Code, data.Type, data.ParentId, data.Path, data.Redirect, data.Icon, data.Component, data.Layout, data.KeepAlive, data.Method, data.Description, data.Show, data.Enable, data.Order)
	return ret, err
}
...
func (m *defaultPermissionModel) Update(ctx context.Context, data *Permission) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.Name, data.Code, data.Type, data.ParentId, data.Path, data.Redirect, data.Icon, data.Component, data.Layout, data.KeepAlive, data.Method, data.Description, data.Show, data.Enable, data.Order, data.Id)
	return err
}
